...
// Inner product
double inp(map<int,int>* x, double* y, int D)
{
  double res = 0;
...
double loss(set<int>& l, set<int>& lhat, int N, int which)
{
  if (l.size() == 0)
  {
  ...
  double precision = (1.0*tp)/lhat.size();
  double recall = (1.0*tp)/l.size();
  if (which == FSCORE)
...
double totalLoss(vector<set<int> >& clusters, vector<set<int> >& chat, int N, int which)
{
