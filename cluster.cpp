                     ...
                     : directed(directed)
{
  ...
  if (nNodes > 1200)
  {
    printf("Sorry this program can handle only upto 1000 nodes!\n");
    exit(0);
  }
...
void Cluster::train(int K, int reps, int gradientReps, int improveReps, double lambda, int seed, int whichLoss)
{
  ...
  for (int rep = 0; rep < reps; rep ++)
  {
    ...
    double ll_prev = loglikelihood(theta, alpha, chat);
...
set<int> Cluster::minimize_graphcuts(int k, int improveReps, int& changed)
{
  ...
  QPBO<double> q(gd->nNodes,E);
  q.AddNode(gd->nNodes);

  map<pair<int,int>, double> mc00;
  map<pair<int,int>, double> mc11;
  vector<pair<double, pair<int,int> > > diff_c00_c11;
  ...
  for (map<pair<int,int>, map<int,int>*>::iterator it = gd->edgeFeatures.begin(); it != gd->edgeFeatures.end(); it ++)
  {
    ...
    double inp_ = inp(it->second, theta + k*gd->nEdgeFeatures, gd->nEdgeFeatures);
    double other_ = 0;
    for (int l = 0; l < K; l ++)
    {
      if (l == k) continue;
      double d = (chat[l].find(e1) != chat[l].end() and chat[l].find(e2) != chat[l].end()) ? 1 : -alpha[l];
    ...
    else diff_c00_c11.push_back(pair<double, pair<int,int> >(-fabs(c00 - c11), it->first));
  }
  ...
  if (gd->nNodes > largestCompleteGraph)
  {
    ...
    for (int i = 0; i < nEdgesToInclude; i ++)
    {
      ...
      double c00 = mc00[edge];
      double c01 = c00;
      double c10 = c00;
      double c11 = mc11[edge];
...
void Cluster::dl(double* dldt, double* dlda, int K, double lambda)
{
  for (int i = 0; i < nTheta; i ++) dldt[i] = -lambda*sgn<double>(theta[i]);
  ...
  for (map<pair<int,int>, map<int,int>*>::iterator it = gd->edgeFeatures.begin(); it != gd->edgeFeatures.end(); it ++)
  {
    ...
    for (int k = 0; k < K; k ++)
    {
      ...
      double d = chatFlat[k][e1] and chatFlat[k][e2] ? 1 : -alpha[k];
      inp_ += d * inps[k];
    }
    double expinp = exp(inp_);
...
double Cluster::loglikelihood(double* theta, double* alpha, vector<set<int> >& chat)
{
  int K = chat.size();
  bool** chatFlat = new bool* [K];
  for (int k = 0; k < K; k ++)
