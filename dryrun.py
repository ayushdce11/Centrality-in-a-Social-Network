...
#   input_file = 'facebook/686'
#   cluster_output = 'results.out_temp'
#   #print "executing: "+"./cluster "+input_file+" "+cluster_output
#   (status,output)=commands.getstatusoutput("./cluster "+input_file+" "+cluster_output+" "+str(i))
#   if status!=0:print output 
#   #else: print "done"
#   print i,'\t',output.split('\n')[-1].split(" ")[-1]
# exit(0)

testrun = "--testrun" in sys.argv or "--skipCluster" in sys.argv
#testrun = True
...
if not testrun:  
  ...
  ouput_file = sys.argv[2] # "./results/"+folder_name+"/"+
  userid = str(input_file.split('/')[-1])
  folder_name = str(userid)
  cluster_output = "./results/"+folder_name+"/temp_output/results.out"
else:
  input_file = "facebook/698"
  userid = str(input_file.split('/')[-1])
  folder_name = str(userid)
  ouput_file = "./results/"+folder_name+"/test_results.out"  
  cluster_output = "./results/"+folder_name+"/temp_output/results.out"
#folder_name = "lastrun123"

#"results/"+str(userid)+"/temp_output"):os.mkdir("results/"+str(userid)+"/temp_output"
if not os.path.isdir("results"):os.mkdir("results")
if not os.path.isdir("results/"+folder_name+""):os.mkdir("results/"+folder_name+"")
if not os.path.isdir("results/"+folder_name+"/temp_output"):os.mkdir("results/"+folder_name+"/temp_output")
...
for line in f:
  ...
  if int(line.split()[0]) not in node_edges: node_edges[int(line.split()[0])]=[]
...
central_nodes = set()
cluster_map = {}
hop_total,hop_count = 0,0
for cluster in clusters:
  cno += 1
  cluster_head = str(cno)#str(userid if int(userid) in cluster else cluster[0])
  if not os.path.isdir("./results/"+folder_name+"/temp_output/"+str(cno)):os.mkdir("./results/"+folder_name+"/temp_output/"+str(cno))
  fedges = open("./results/"+folder_name+"/temp_output/"+str(cno)+"/"+cluster_head+".edges",'w')
  fcircles = open("./results/"+folder_name+"/temp_output/"+str(cno)+"/"+cluster_head+".circles",'w')
  fegofeat = open("./results/"+folder_name+"/temp_output/"+str(cno)+"/"+cluster_head+".egofeat",'w')
  ffeat = open("./results/"+folder_name+"/temp_output/"+str(cno)+"/"+cluster_head+".feat",'w')
  ...
  #   else: print "done"
  # else:
  print "executing: "+"./spanning results/"+folder_name+"/temp_output/"+str(cno)+"/"+str(cluster_head)+".edges"
  (status,output)=commands.getstatusoutput("./spanning results/"+folder_name+"/temp_output/"+str(cno)+"/"+str(cluster_head)+".edges")
  if status!=0:print output 
  else: print "done"
  ...
  #get hopcounts
  (status,output)=commands.getstatusoutput("./hops results/"+folder_name+"/temp_output/"+str(cno)+"/"+str(cluster_head)+".edges "+str(output))
  if float(output.split('\n')[0])>=0:
    print "Average Hop Count for this cluster: ",float(output.split('\n')[0])
    hop_total += float(output.split('\n')[0])
    hop_count += 1

print "Set of Target Nodes are ",central_nodes

print "Average Hop Count: "+str(float(hop_total)/hop_count)

graph = pydot.Dot(graph_type='graph')

f = open("results/"+folder_name+"/output.edges",'w')
    ...
    for cluster_node in node_edges[curr]:
      if cluster_node not in visi:
        ...
        if cluster_node != node and cluster_node in cluster_map[node]:
          ...
          if (str(curr), str(cluster_node)) not in pic_edges and (str(cluster_node),str(curr)) not in pic_edges:
           pic_edges.add(edge)
  
  for cluster_node in cluster_map[node]: #add nodes that are not accessible from cluster head
    if cluster_node not in visi:
      edge = (str(node), str(cluster_node))
      print edge
      pic_edges.add(edge)
...
graph.write_png("results/"+folder_name+"/output_graph.png")
